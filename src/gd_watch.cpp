...
void gd_watch_arm_exit(DWORD pid, const char *exe_lower) {
	...
	if (!hp) {
		blog(LOG_DEBUG,
		     "[obs-game-detector] exit wait: pid %lu already gone",
		     (unsigned long)pid);
	...
	if (!slot) {
		...
		CloseHandle(hp);
		blog(LOG_ERROR,
		     "[obs-game-detector] exit wait table full, pid %lu untracked",
		     (unsigned long)pid);
		return;
	}
	...
	if (!RegisterWaitForSingleObject(&slot->wait, hp, on_process_exit, slot,
	                                 INFINITE, WT_EXECUTEONLYONCE)) {
		blog(LOG_ERROR,
		     "[obs-game-detector] RegisterWaitForSingleObject failed for pid %lu",
		     (unsigned long)pid);
		exit_slot_clear(slot);
		LeaveCriticalSection(&s_exit_lock);
		return;
...
static bool get_target_process(IWbemClassObject *event, IWbemClassObject **out_proc) {
	...
	HRESULT hr =
		vt.punkVal->QueryInterface(IID_IWbemClassObject, (void **)out_proc);
	VariantClear(&vt);
	return SUCCEEDED(hr);
}

static bool read_process_event(IWbemClassObject *event, char *exe_lower,
                               size_t exe_cap, DWORD *pid_out) {
	...
	WideCharToMultiByte(CP_ACP, 0, v.bstrVal, -1, exe_lower, (int)exe_cap,
	                    NULL, NULL);
	VariantClear(&v);
	proc->Release();
...
public:
	...
	HRESULT STDMETHODCALLTYPE Indicate(LONG count,
		...
		for (LONG i = 0; i < count; i++) {
			...
			if (!read_process_event(objs[i], exe_lower, sizeof(exe_lower),
			                        &pid))
				continue;
...
static bool setup_wmi(void) {
	...
	hr = pLoc->ConnectServer(_bstr_t(L"ROOT\\CIMV2"), NULL, NULL, NULL, 0,
	                         NULL, NULL, &s_wmi_svc);
	pLoc->Release();
	if (FAILED(hr))
		return false;

	hr = CoSetProxyBlanket(s_wmi_svc, RPC_C_AUTHN_WINNT, RPC_C_AUTHZ_NONE, NULL,
	...
	BSTR query = SysAllocString(
		L"SELECT * FROM __InstanceCreationEvent WITHIN 0.1 "
		L"WHERE TargetInstance ISA 'Win32_Process'");

	hr = s_wmi_svc->ExecNotificationQueryAsync(
		lang, query, WBEM_FLAG_SEND_STATUS, NULL, &g_creation_sink);
...
static DWORD WINAPI wmi_thread_func(LPVOID) {
	...
	if (SUCCEEDED(hr) || hr == (HRESULT)RPC_E_CHANGED_MODE) {
		if (setup_wmi()) {
			blog(LOG_INFO,
			     "[obs-game-detector] WMI process creation watch active");
		} else {
			blog(LOG_ERROR,
			     "[obs-game-detector] WMI process creation watch failed");
		}
	} else {
		blog(LOG_ERROR,
		     "[obs-game-detector] WMI thread COM init failed (0x%08lx)",
		     (unsigned long)hr);
	}
...
void gd_watch_snapshot(const GD_LookupTable *lt) {
	...
	if (n == 0) {
		blog(LOG_ERROR,
		     "[obs-game-detector] boot snapshot: process scan failed");
		return;
	}
	...
	for (int i = 0; i < n; i++) {
		if (lt && !gd_lookup_matches_full_path(lt, s_snapshot_procs[i].path))
			continue;
		hits++;
		post_start(s_snapshot_procs[i].pid, s_snapshot_procs[i].exe,
		           s_snapshot_procs[i].path);
	}

	blog(LOG_INFO,
	     "[obs-game-detector] boot snapshot: %d lookup hits / %d with paths",
	     hits, n);
}
