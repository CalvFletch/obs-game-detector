...
static int tracker_scan(GD_Tracker *t, GD_GameId id, DWORD pid, bool match_id,
                        bool match_pid, tracker_match_fn fn, void *ctx) {
	int n = 0;
...
static GD_TrackedGame *tracker_find(GD_Tracker *t, GD_GameId id, DWORD pid,
                                    int mode) {
...
static void tracker_foreach_by_id(GD_Tracker *t, GD_GameId id,
                                  tracker_visit_fn fn, void *ctx) {
	for (int i = 0; i < t->count; i++)
		if (t->games[i].id == id)
			fn(&t->games[i], ctx);
}
...
static void make_obs_source_name(GD_State *state, const char *display_name,
                                 DWORD pid, GD_GameId id, char *out,
                                 size_t cap) {
	int same_id = tracker_count_by_id(&state->tracker, id);
	bool need_pid =
		same_id > 0 || (tracker_find(&state->tracker, id, pid, TRACK_BY_BOTH) != NULL);
...
static void remove_audio_by_id(GD_State *state, GD_GameId id, DWORD pid,
...
static void coalesce_events(GD_Event *evts, int *count) {
	...
	for (int i = 0; i < *count; i++) {
		...
		if (e.kind == GD_EVT_SYNC_SCENES || e.kind == GD_EVT_APPLY_TRACKS) {
			...
			for (int j = 0; j < n; j++) {
				if (evts[j].kind == GD_EVT_REMOVE_BY_ID &&
				    evts[j].game_id == e.game_id) {
					merged = true;
					break;
				}
			}
			...
			for (int j = 0; j < n; j++) {
				if (evts[j].kind == GD_EVT_PLACE_AUDIO &&
				    evts[j].game_id == e.game_id) {
					evts[j].pid = e.pid;
					merged  = true;
					break;
			...
			for (int j = 0; j < n; j++) {
				if (evts[j].kind == GD_EVT_PROCESS_START &&
				    evts[j].pid == e.pid) {
					evts[j] = e;
					merged  = true;
					break;
...
static void handle_process_start(GD_State *state, const GD_Event *evt) {
	...
	if (!full_path || !full_path[0]) {
		blog(LOG_ERROR,
		     "[obs-game-detector] no image path for pid %lu %s: untracked",
		     (unsigned long)evt->pid, exe_lower);
		return;
	}
	...
	if (!gd_lookup_matches_full_path(&state->lookup, full_path)) {
		if (gd_skip_heuristic(exe_lower))
			return;
		blog(LOG_DEBUG, "[obs-game-detector] SKIP (not game path) %s",
		     exe_lower);
		return;
	}
	...
	if (!gd_lookup_resolve(state, full_path, &id, display_name,
	                       sizeof(display_name), install_dir,
	                       sizeof(install_dir))) {
		blog(LOG_ERROR,
		     "[obs-game-detector] install index miss for %s: untracked",
		     full_path);
		return;
	}
	...
	GD_TrackedGame *g =
		tracker_find(&state->tracker, id, 0, TRACK_BY_GAME_ID);
	if (g) {
		if (g->pid == evt->pid)
			return;

		blog(LOG_INFO,
		     "[obs-game-detector] START pid %lu %s -> %s (pid %lu -> %lu)",
		     (unsigned long)evt->pid, exe_lower, g->obs_source_name,
		     (unsigned long)g->pid, (unsigned long)evt->pid);
		...
		if (state->tracker.count >= GD_MAX_GAMES) {
			blog(LOG_ERROR,
			     "[obs-game-detector] tracked game cap (%d) reached: ignoring pid %lu",
			     GD_MAX_GAMES, (unsigned long)evt->pid);
			return;
		}

		char obs_name[256];
		make_obs_source_name(state, display_name, evt->pid, id, obs_name,
		                     sizeof(obs_name));
		...
		blog(LOG_INFO, "[obs-game-detector] START pid %lu %s -> %s",
		     (unsigned long)evt->pid, exe_lower, g->obs_source_name);
	}
...
static void handle_process_stop(GD_State *state, const GD_Event *evt) {
	GD_TrackedGame *g =
		tracker_find(&state->tracker, 0, evt->pid, TRACK_BY_PID);
	if (!g)
		return;

	blog(LOG_INFO, "[obs-game-detector] STOP pid %lu %s -> %s",
	     (unsigned long)evt->pid, evt->exe_lower, g->obs_source_name);
	...
	for (int i = 0; i < state->tracker.count; i++) {
		if (state->tracker.games[i].pid == evt->pid) {
			state->tracker.games[i] =
				state->tracker.games[--state->tracker.count];
			break;
		}
	}
}
...
static void maybe_schedule_index_rebuild(GD_State *state) {
	if (!state || state->phase != GD_PHASE_RUNNING ||
	    state->index_rebuild_pending)
		return;

	uint64_t now_ms = gd_wall_ms();
	if (now_ms <= state->index_built_ms ||
	    now_ms - state->index_built_ms <= GD_INDEX_REBUILD_MS)
		return;
...
static void processor_tick(void *unused) {
	...
	if (!state || state->phase == GD_PHASE_IDLE ||
	...
	if (state->obs_processor_depth == 0 && event_pending() &&
...
static void obs_mut_remove_capture(obs_source_t *src) {
	...
	blog(LOG_INFO, "[obs-game-detector] removing capture '%s'",
	     name ? name : "?");
	if (name)
...
static void get_target_scenes(const GD_ConfigSnap *cfg,
                              char scenes[][GD_MAX_SCENE_LEN], int *count,
...
static void on_group_item_removed(void *data, calldata_t *cd) {
	...
	GD_TrackedGame *g =
		tracker_find(&state->tracker, 0, spid, TRACK_BY_PID);
...
static void reinstate_disconnect(GD_TrackedGame *g) {
	...
	if (grp_src) {
		if (obs_source_get_id(grp_src) &&
		    strcmp(obs_source_get_id(grp_src), "group") == 0) {
			signal_handler_disconnect(
				obs_source_get_signal_handler(grp_src),
				"item_remove", on_group_item_removed,
				(void *)(uintptr_t)hook_pid);
		}
...
static obs_source_t *get_group_source(bool log_wrong_type) {
	...
	if (!is_group_source(grp_src)) {
		if (log_wrong_type) {
			blog(LOG_ERROR,
			     "[obs-game-detector] '%s' exists but is not a group source",
			     GD_GROUP_NAME);
		}
		obs_source_release(grp_src);
		return NULL;
...
static bool find_or_scrub_group_item(obs_scene_t *, obs_sceneitem_t *item,
                                     void *vp) {
	...
	const char *name = obs_source_get_name(src);
	bool match =
		name && ctx->source_name && strcmp(name, ctx->source_name) == 0;
	if (match) {
		ctx->found = item;
		return false;
	}
...
static obs_sceneitem_t *find_live_item_in_group(obs_scene_t *grp_scene,
                                                const char *source_name) {
...
static bool find_group_item_by_game_id(obs_scene_t *, obs_sceneitem_t *item,
                                       void *vp) {
...
static void visit_gd_captures(GD_GameId filter_id, gd_capture_visit_fn visitor,
                              void *userdata) {
...
static void pending_rm_add(gd_pending_rm_t *pending, obs_source_t *src) {
	if (!pending)
		return;
	if (pending->count >=
	    (int)(sizeof(pending->list) / sizeof(pending->list[0]))) {
		const char *name = obs_source_get_name(src);
		blog(LOG_ERROR,
		     "[obs-game-detector] pending_rm full: dropping '%s'",
		     name ? name : "?");
		return;
	}
...
static bool queue_capture_rm_visit(obs_source_t *src, void *vp) {
	...
	const char *name = obs_source_get_name(src);
	blog(LOG_INFO, "[obs-game-detector] queue capture '%s' for removal",
	     name ? name : "?");
	pending_rm_add(ctx->pending, src);
	return true;
}

static void queue_gd_captures_for_removal(GD_GameId filter_id,
	...
	if (!should_queue) {
		...
		auto selective_visit = +[](obs_source_t *src, void *vp) -> bool {
			...
			const char *name = obs_source_get_name(src);
			blog(LOG_INFO,
			     "[obs-game-detector] queue capture '%s' for removal",
			     name ? name : "?");
...
static void dedupe_captures_for_game_id(GD_GameId id, obs_source_t *keep) {
	...
	auto visit = +[](obs_source_t *src, void *vp) -> bool {
		...
		blog(LOG_INFO,
		     "[obs-game-detector] queue duplicate capture '%s' for removal",
		     dup_name ? dup_name : "?");
...
static bool reconcile_queue_visit(obs_source_t *src, void *vp) {
	...
	blog(LOG_INFO, "[obs-game-detector] boot reconcile: queue stale '%s'",
...
static bool apply_tracks_visit(obs_source_t *src, void *vp) {
	...
	obs_mut_assert_active();
	uint32_t mixers =
		gd_config_mixer_mask(&state->config, source_game_id(src));
...
static void apply_wasapi_capture(obs_source_t *source, GD_TrackedGame *g,
	...
	if (ext)
		*ext = '\0';
	char window_str[512];
	snprintf(window_str, sizeof(window_str), "%s:.:%s", cap_title,
	         g->capture_exe);
...
static obs_source_t *ensure_capture_source(GD_TrackedGame *g,
                                           const GD_ConfigSnap *cfg) {
	obs_mut_assert_active();
	...
	if (!source) {
		...
		source = obs_source_create("wasapi_process_output_capture",
		                           g->obs_source_name, settings, NULL);
		obs_data_release(settings);
	}
	if (!source)
		return NULL;
...
static bool place_capture_in_group(obs_source_t *source,
                                   char scenes[][GD_MAX_SCENE_LEN],
                                   int scene_count) {
	obs_mut_assert_active();
	...
	for (int i = 0; i < scene_count; i++) {
		...
		if (scene) {
			if (!grp_src)
				grp_src = obs_source_create("group", GD_GROUP_NAME,
				                            NULL, NULL);
	...
	if (placed) {
		...
		if (grp_refresh) {
			obs_scene_t *grp_scene =
				obs_group_from_source(grp_refresh);
			if (grp_scene) {
				calldata_t cd = {};
...
static void place_audio(GD_State *state, GD_TrackedGame *g, bool notify) {
	...
	if (!gd_config_is_enabled(cfg, g->id)) {
		blog(LOG_INFO, "[obs-game-detector] '%s' is disabled: skipping add",
		     g->obs_source_name);
		return;
	}
...
static void remove_audio_by_id(GD_State *state, GD_GameId id, DWORD pid,
	...
	GD_TrackedGame *g =
		tracker_find(&state->tracker, id, pid, TRACK_BY_BOTH);
	if (g) {
		remove_audio(state, g);
		return;
	}
...
static void sync_group_to_scenes(void) {
	...
	if (grp_src) {
		...
		for (int i = 0; i < sc_count; i++) {
			...
			if (scene) {
				obs_sceneitem_t *it =
					obs_scene_find_source(scene, GD_GROUP_NAME);
				if (it)
					obs_mut_sceneitem_remove(it);
			}
...
void gd_event_post(const GD_Event *evt) {
	...
	if (next == ring->head) {
		pthread_mutex_unlock(&ring->mutex);
		blog(LOG_ERROR,
		     "[obs-game-detector] event queue full: event %d",
		     (int)evt->kind);
		return;
	}
...
void gd_start(void) {
	...
	if (gd_install_index_cache_path(index_cache, sizeof(index_cache))) {
		...
		state->index_built_ms =
			gd_install_index_normalize_built_ms(state->index.built_ms);
	} else {
